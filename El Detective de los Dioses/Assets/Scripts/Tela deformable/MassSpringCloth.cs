...
public class MassSpringCloth : MonoBehaviour
{
    ...
    public struct EdgeCloth : IComparable<EdgeCloth>
    {
        ...
        public int CompareTo(EdgeCloth other)
        {
    ...
    public List<EdgeCloth> aristas = new List<EdgeCloth>(); // Lista que va a almacenar todas las aristas
    ...
    void Start()
    {
        ...
        nodos = new List<NodeCloth>(vertices.Length);
        foreach(Vector3 v in vertices)
        {
            nodos.Add(new NodeCloth((masaTotal/vertices.Length), transform.TransformPoint(v), dAbsolute)); // La masa de cada nodo será la misma -> masaTotal / numNodos
    ...
    private void FixedUpdate()
    {
        ...
        foreach (SpringCloth muelle in muelles)
        {
            // Dirección
    ...
    private void inicializarMuelles()
    {
        ...
        for(int i=0; i<triangulos.Length; i+=3)
        {
            ...
            EdgeCloth arista_AB, arista_BC, arista_AC;
            if(vertexA < vertexB)
            {
                arista_AB = new EdgeCloth(vertexA, vertexB, vertexC);
            }
            else
            {
                arista_AB = new EdgeCloth(vertexB, vertexA, vertexC);
            }

            if(vertexB < vertexC)
            {
                arista_BC = new EdgeCloth(vertexB, vertexC, vertexA);
            }
            else
            {
                arista_BC = new EdgeCloth(vertexC, vertexB, vertexA);
            }

            if(vertexA < vertexC)
            {
                arista_AC = new EdgeCloth(vertexA, vertexC, vertexB);
            }
            else
            {
                arista_AC = new EdgeCloth(vertexC, vertexA, vertexA);
            }
        ...
        for(int i=0; i<aristas.Count-1; i++)
        {
            ...
            {
                SpringCloth flexion = new SpringCloth(k_Flexion, nodos[aristas[i].vertexOther], nodos[aristas[i + 1].vertexOther], dDeformation); // Se crea un muelle de flexión entre los vértices
            ...
            {
                SpringCloth traccion = new SpringCloth(k_Traccion, nodos[aristas[i].vertexA], nodos[aristas[i].vertexB], dDeformation); // Se crea un muelle de tracción entre los vértices de la arista
        ...
        muelles.Add(new SpringCloth(k_Traccion, nodos[aristas[aristas.Count-1].vertexA], nodos[aristas[aristas.Count-1].vertexB], dDeformation));
    }
    ...
    private void applyDampingSpring(SpringCloth muelle)
    {
